...
(function() {
  ...
  let progressListener = {
    ...
    onLocationChange: function onLocationChange(progress, request,
      ...
      function generateStubs(window) {
        // XXX Generate those lazily
        createMozSettings(window);
        createMozApps(window);
        createMozContacts(window);
      ...
      content.addEventListener('appwillopen', function(evt) {
        ...
        generateStubs(topWindow);
      });

      generateStubs(content.wrappedJSObject);
    },
...
function createMozSettings(window) {
  try {
  ...
  handleEvent: function th_handleEvent(evt) {
    ...
    switch (evt.type) {
      ...
      case 'touchmove':
        ...
        break;
      case 'contextmenu':
        var sourceURL = (evt.target.ownerDocument || this._window.document).URL;
...
function createMozContacts(window) {
