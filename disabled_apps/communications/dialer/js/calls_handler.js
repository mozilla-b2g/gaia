...
var CallsHandler = (function callsHandler() {
  ...
  function setup() {
    if (telephony) {
      telephony.addEventListener('callschanged', onCallsChanged);
      onCallsChanged(); // The handler is invoked explicitly to prevent races
  ...
  function handleCommand(evt) {
    if ((evt.origin !== COMMS_APP_ORIGIN) || !evt.data) {
      return;
    }
  ...
  return {
    ...
    // Exposed for unit-testing until we find a better way
