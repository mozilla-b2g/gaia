...
ifneq ($(strip $(APP)),)
	...
	@./node_modules/test-agent/bin/js-test-agent test $(TEST_ARGS) --server ws://localhost:$(TEST_AGENT_PORT) -t "$(GAIA_ALLAPPDIRS)" --reporter $(REPORTER) $(APP_TEST_LIST)
else
	@echo 'Running all tests';
	@./node_modules/test-agent/bin/js-test-agent test $(TEST_ARGS) --server ws://localhost:$(TEST_AGENT_PORT) -t "$(GAIA_ALLAPPDIRS)" --reporter $(REPORTER)
endif

.PHONY: test-agent-server
test-agent-server: common-install node_modules
	@./node_modules/test-agent/bin/js-test-agent server --port $(TEST_AGENT_PORT) -c ./shared/test/unit/test-agent-server.js -t "$(GAIA_ALLAPPDIRS)" --http-path . --growl
