...
(function (exports) {
   ...
   */
  function map2dIs(thisObj, manifestURL, layoutId) {
    // force boolean true or false
    return !!(thisObj[manifestURL] && thisObj[manifestURL][layoutId]);
  }
  /**
   * helper function for setting a value to true in one of the currentSettings
   */
  function map2dSet(thisObj, manifestURL, layoutId) {
    if (!thisObj[manifestURL]) {
      thisObj[manifestURL] = {};
    }
    thisObj[manifestURL][layoutId] = true;
  }
  /**
   * helper function for setting a value to false in one of the currentSettings
   * deletes the appropriate keys
   */
  function map2dUnset(thisObj, manifestURL, layoutId) {
    if (!thisObj[manifestURL]) {
      return;
    }
    delete thisObj[manifestURL][layoutId];
    if (!Object.keys(thisObj[manifestURL]).length) {
      delete thisObj[manifestURL];
    }
  }
  /**
   ...
   */
  function kh_parse3rdPartyAppEnabled(evt) {
    var value = evt.target.result[SETTINGS_KEYS.THIRD_PARTY_APP_ENABLED];
   ...
   */
  function kh_parseDefault(evt) {
    var value = evt.target.result[SETTINGS_KEYS.DEFAULT];
    if (value) {
   ...
   */
  function kh_parseCurrentActive(evt) {
    var value = evt.target.result[SETTINGS_KEYS.CURRENT_ACTIVE];
    if (value) {
   ...
   */
  function kh_parseEnabled(evt) {
    var value = evt.target.result[SETTINGS_KEYS.ENABLED];
    var needsSave = false;
    if (!value) {
        ...
        try {
          ...
          oldSettings.forEach(function (layout) {
            if (layout.enabled) {
              ...
              map2dSet(
                currentSettings.enabledLayouts, manifestURL, layout.layoutId);
            }
          });
        } catch (e) {
          currentSettings.enabledLayouts =
  ...
  function kh_migrateDeprecatedSettings(deprecatedSettings) {
    ...
    Object.keys(DEPRECATE_KEYBOARD_SETTINGS).forEach(function(key) {
      ...
      if (deprecatedSettings[settingEntry]) {
        ...
        if (key in MULTI_LAYOUT_MAP) {
          MULTI_LAYOUT_MAP[key].forEach(function enableLayout(layoutId) {
            map2dSet(currentSettings.enabledLayouts,
    ...
    if (!hasEnabledLayout) {
      map2dSet(
        currentSettings.enabledLayouts, defaultKeyboardManifestURL, 'en');
    }
    // Clean up all the deprecated settings
    var deprecatedSettingsQuery = {};
    for (var key in DEPRECATE_KEYBOARD_SETTINGS) {
      if (DEPRECATE_KEYBOARD_SETTINGS.hasOwnProperty(key)) {
  ...
  function kh_getMultiSettings(settings, callback) {
    ...
    try {
      ...
      console.warn(
        'Exception in mozSettings.createLock():', e, '\nUsing default values');
    ...
    function requestSetting(name) {
      ...
      try {
        ...
        console.warn(
          'Exception querying setting', name, ':', e, '\nUsing default value');
  ...
  var KeyboardHelper = exports.KeyboardHelper = {
    ...
    init: function kh_init() {
      ...
      if (settings) {
        ...
        kh_getMultiSettings(
          deprecatedSettingsQuery, kh_migrateDeprecatedSettings);
      }
     ...
     */
    setLayoutEnabled: function kh_setLayoutEnabled(manifestURL,
                                                   layoutId, enabled) {
      var method = enabled ? map2dSet : map2dUnset;
      method(currentSettings.enabledLayouts, manifestURL, layoutId);
    },
    /**
    ...
    getLayoutEnabled: function kh_getLayoutEnabled(manifestURL, layoutId) {
      return map2dIs(currentSettings.enabledLayouts, manifestURL, layoutId);
    },
    /**
     * set/unset a layout as default based on manifest URL and layoutId
     */
    setLayoutIsDefault: function kh_setLayoutIsDefault(manifestURL,
                                                       layoutId, enabled) {
      var method = enabled ? map2dSet : map2dUnset;
      method(currentSettings.defaultLayouts, manifestURL, layoutId);
    },
    /**
    ...
    getLayoutIsDefault: function kh_getLayoutIsDefault(manifestURL, layoutId) {
      return map2dIs(currentSettings.defaultLayouts, manifestURL, layoutId);
    },
    /**
    ...
    saveToSettings: function ke_saveToSettings() {
      ...
      toSet[SETTINGS_KEYS.CURRENT_ACTIVE] =
        currentSettings.currentActiveLayouts;
      window.navigator.mozSettings.createLock().set(toSet);
    },
    /**
     * Get a list of current keyboard applications.  Will call callback
     * immediately if the data is already cached locally.  Will not call
     * callback if for some reason no apps are found.
     */
    getApps: function kh_getApps(callback) {
      ...
      navigator.mozApps.mgmt.getAll().onsuccess = function onsuccess(event) {
        ...
        if (keyboardApps.length) {
          ...
          Object.keys(currentSettings.enabledLayouts).concat(
            Object.keys(currentSettings.defaultLayouts)).forEach(
    ...
    getLayouts: function kh_getLayouts(options, callback) {
      ...
      var withApps = function withApps(apps) {
        ...
        for (var group in this.fallbackLayoutNames) {
          if (this.fallbackLayoutNames.hasOwnProperty(group)) {
        ...
        var layouts = apps.reduce(function eachApp(result, app) {
          ...
          Object.keys(manifest.inputs, function(layoutId) {
          ...
          if (options['default']) {
            result.sort(function (a, b) {
              return a.inputManifest.types.length -
                     b.inputManifest.types.length;
            });
          }
          return result;
        }.bind(this), []);
     ...
     * passed onto getLayouts. The callback will be called as soon as the
     * current settings and apps are loaded as well.
     *
      ...
      this.getDefaultLayoutConfig(function gotDefaultLayouts(keyboards) {
        ...
        for (var i = langIndependentLayouts.length - 1; i >= 0; i--) {
          this.setLayoutIsDefault(kbManifestURL,
            langIndependentLayouts[i].layoutId, true);
        }
        if (reset) {
          // reset the set of default layouts
          currentSettings.enabledLayouts =
        ...
        for (i = newKbLayouts.length - 1; i >= 0; i--) {
          this.setLayoutIsDefault(
            kbManifestURL, newKbLayouts[i].layoutId, true);
