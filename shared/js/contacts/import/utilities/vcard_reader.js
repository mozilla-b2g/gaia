  ...
  */
  ...
  window.VCardReader = function VCardReader(data) {
    ...
    this.next = function() {
      LazyLoader.load(dependencies, function() {
        _readNext(function(contact) {
          console.log('READING CONTACT AT:', currentIndex);
          if (this.onContactParsed && contact) {
            this.onContactParsed(contact); 
          }
        }.bind(this));
      }.bind(this));
    };
    ...
    function _readNext(cb) {
      var fields = {};
      var lines = _getCardLines();
      var line;
      var parsedLine;

      // look if there is more cards to read
      if (!content.substr(currentIndex).match(/end:vcard/gi)) {
        return;
      }
