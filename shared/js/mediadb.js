  ...
  function MediaDB(mediaType, metadataParser, options) {
    ...
    function enumerateOldFiles(store, callback) {
      ...
      getAllRequest.onsuccess = function() {
        var dbfiles = getAllRequest.result;
        callback(dbfiles);
      };
    }
    ...
    function handleUpgrade(db, trans, oldDbVersion, oldClientVersion) {
      ...
      enumerateOldFiles(store, function doUpgrade(dbfiles) {
        if (!dbfiles || !dbfiles.length) {
          return;
        }
        // handle mediadb upgrade from 2 to 3
        if (2 == oldDbVersion && 3 == MediaDB.VERSION) {
          upgradeDBVer2to3(store, dbfiles);
        }
        // handle client upgrade
        if (clientUpgradeNeeded) {
          handleClientUpgrade(store, dbfiles, oldClientVersion);
        }
      });
    }

    function handleClientUpgrade(store, dbfiles, oldClientVersion) {
    ...
    function upgradeDBVer2to3(store, dbfiles) {
      var prefix = '/sdcard/'; // prefix of full qualified name.

      // delete and add all of them in a single transaction.
      for (var i = 0; i < dbfiles.length; i++) {
        var dbfile = dbfiles[i];
        ...
        dbfile.name = prefix + dbfile.name;
        store.add(dbfile);
      }
    }
