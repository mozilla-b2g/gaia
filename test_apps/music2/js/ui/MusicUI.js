...
var MusicUI = function() {
  ...
  this.viewEvents.router.when('startDragCurrentMusicPage',
    [this.viewVisibility, 'startDragCurrentMusicPage']
  );

  this.viewEvents.router.when('snapPositionCurrentMusicPage',
    [this.viewVisibility, 'snapPositionCurrentMusicPage']
  );
  ...
  this.mediaLibraryPage.router.when('requestAddSongs', function(title, songs) {
    ...
    this.currentMusicPage.controls.nowPlaying.temporarilyShowText(
      'added ' + title, 1000
    );
  }.bind(this));

  this.mediaLibraryPage.router.when(
    'requestFullRefresh', [this, '_setFullRefresh']
  );
  ...
  this.mediaLibraryPage.router.when('registerPlaylistsUpdateListener',
    function(fn) {
      this.playlistsUpdateListeners.push(fn);
    }.bind(this)
  );

  this.mediaLibraryPage.router.when('registerCurrentPlaylistUpdateListener',
    function(fn) {
      this.currentPlaylistUpdateListeners.push(fn);
    }.bind(this)
  );

  this.mediaLibraryPage.router.when('registerPlaylistUpdateListener',
    function(playlistId, fn) {
...
MusicUI.prototype = {
  ...
  setPlaylist: function(playlist, playlistId) {
    ...
    this.currentPlaylistUpdateListeners.forEach(
      function(fn) { fn(playlist, playlistId); }
    );
    if (this.playlistUpdateListeners[playlistId])
      this.playlistUpdateListeners[playlistId].forEach(
        function(fn) { fn(playlist, playlistId); }
      );
  ...
  musicChanged: function(numberCreated, numberDeleted) {
    if (!this.fullRefresh) {
      this.mediaLibraryPage.userWantRefresh(numberCreated, numberDeleted,
  ...
  _setupEventViewEvents: function() {
    ...
    for (var event in eventViewTable) {
      this.viewEvents.router.when(
        event, [this.viewVisibility, eventViewTable[event]]
      );
    }
  },
