...
var CurrentMusicPageSource = function() {
  ...
  Utils.onButtonTap(
    this.dom.currentSourceFavorite, this.toggleFavorite.bind(this)
  );
};
...
CurrentMusicPageSource.prototype = {
  ...
  toggleInfoVisibility: function() {
    if (!this.dom.currentSourceInfo.classList.contains('hidden')) {
      ...
      Utils.runEventOnce(this.dom.currentSourceInfo, 'transitionend',
        function() {
      ...
      var transformY = 'translateY(' + y + 'px)';
      this.dom.currentSourceFavorite.style.transform = transformY;

      Utils.runEventOnce(this.dom.currentSourceFavorite, 'transitionend',
        function() {
    ...
    if (source) {
      ...
      this.clearListener =
        window.musicLibrary.musicDB.registerSongFavoriteChangeListener(
          source.song, this._updateIsFavorite.bind(this)
        );
    }
    else {
