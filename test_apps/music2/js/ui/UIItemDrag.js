...
UIItemDrag.prototype = {
  ...
  stop: function() {
    ...
    if (this.state.currentDropDir === 'above') {
      ...
      this.dom.list.insertBefore(
        this.dom.div, this.state.currentDropElement.nextElementSibling
      );
    }
    else {
      console.warn(
        'this.state.currentDropDir in bad state: ' + this.state.currentDropDir
      );
    }

    this.router.route('moved')(
      this.state.currentDropElement, this.state.currentDropDir
    );
  },
  ...
  setDropPoint: function(listRect, x, y) {
    ...
    if (y < listRect.top) {
      ...
      var threshold = this.dom.list.scrollTop + this.dom.list.offsetHeight;
      while (next.offsetTop >= threshold) {
  ...
  updateScroll: function() {
    ...
    if (this.state.currentScroll >
        this.dom.list.scrollHeight - this.dom.list.clientHeight)
    {
