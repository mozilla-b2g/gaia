...
PlaylistView.prototype = {
  ...
  setPlaylist: function(playlist, mode) {
    ...
    if (playlist === null || playlist === undefined) {
      ...
      text.innerHTML =
        'no playlist selected:<br>select songs or select a playlist';
      ...
      this.extraOptions.show({
        ...
        addTo: function() {
          this.router.route('requestAddSongsToCustom')(
            source.song.metadata.title, [source.song]
          );
        }.bind(this),
  ...
  onElemMoved: function(moveData) {
    ...
    this.router.route('movePlaylistItemRelative')(
      this.currentPlaylist, item.data.source,
      relativeItem.data.source, relativeDir
    );
  }
};
