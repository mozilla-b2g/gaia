...
var MediaLibraryPage = function() {//pageBridge){
  ...
  this.panelManager = new MediaLibraryPanelManager();
  //this.musicDB, this.pageBridge);
...
MediaLibraryPage.prototype = {
  ...
  createMusicPanelFromItems: function(query, items) {
    var panel =
      MediaLibraryMusicPanelFactory.createMediaLibraryMusicPanel(
        query, items, this.mode
      );
  ...
  createPlaylistsPanel: function() {
    ...
    var panel = new MediaLibraryPlaylistsPanel(
      playlists, currentPlaylist.playlist, currentPlaylist.playlistId
    );

    this.router.route('registerPlaylistsUpdateListener')(
      panel.updatePlaylists.bind(panel)
    );
    this.router.route('registerCurrentPlaylistUpdateListener')(
      panel.updateCurrentPlaylist.bind(panel)
    );
    this.setupPanel(panel);
    this.panelManager.pushPanel(panel);
  },
  createPlaylistPanel: function(playlist, id) {
    var currentPlaylist = this.router.route('getPlaylist')();
    var panel = new MediaLibraryPlaylistPanel(
      playlist, id, currentPlaylist.playlistId
    );

    this.router.route('registerPlaylistUpdateListener')(
      id, panel.updatePlaylist.bind(panel)
    );
    this.router.route('registerCurrentPlaylistUpdateListener')(
      panel.updateCurrentPlaylist.bind(panel)
    );
