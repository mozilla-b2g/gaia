...
PlayingPlaylist.prototype = {
  ...
  switchToSources: function(title, sources){
    this.stop();
    sources = sources.map(function(song){ return this.router.route('requestSourceFromSong')(song); }.bind(this));
  ...
  savePlaylist: function(){
    this.playlistId = this.router.route('savePlaylistToPlaylists')(this.playlist, null);
    this._setPlaylist();
    this.router.route('becameNotSavable')();
  },
  ...
  _songStateChanged: function(dontUpdateInfo){
    ...
    if (this.playlist.list.length !== 0){
      if (this._atEnd() || this._atBegin())
        ...
        song = this.playlist.list[this.index];
      this.router.route('songStateUpdated')(song, this.index, this.state, dontUpdateInfo);
    }
    if (this._atEnd()){
