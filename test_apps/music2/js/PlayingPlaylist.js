...
PlayingPlaylist.prototype = {
  ...
  switchToSources: function(title, sources){
    this.stop();
    sources = sources.map(function(song){ return this.requestSourceFromSong(song); }.bind(this));
  ...
  savePlaylist: function(){
    this.playlistId = this.savePlaylistToPlaylists(this.playlist, null);
    this._setPlaylist();
    this.becameNotSavable();
  },
  movedItemInPlaylist: function(playlistId, source, relativeSource, relativeDir){
    if (this._playlistMatchesThis(playlistId)){
  ...
  _setPlaylist: function(){
    if (this.mode === 'simple')
      this.playlistUpdated(this.playlist, null);
    else 
      this.playlistUpdated(this.playlist, this.playlistId);
  },
  ...
  _save: function(){
    if (this.playlistId !== null){
      this._setPlaylist(); //TODO inefficient
      this.save(this.playlist, this.playlistId);
    }
  },
  _updateMode: function(mode){
    if (this.mode !== mode){
      this.mode = mode;
      this.modeUpdated(this.mode);
    }
  },
  _songStateChanged: function(dontUpdateInfo){
    ...
    if (this.playlist.list.length !== 0){
      if (this._atEnd() || this._atBegin())
        ...
        song = this.playlist.list[this.index];
      this.songStateUpdated(song, this.index, this.state, dontUpdateInfo);
    }
    if (this._atEnd()){
      this.cantGoNext();
    }
