...
class GaiaData(object):
    ...
    def delete_all_sms(self):
        ...
        with self.marionette.using_context('chrome'):
            self.marionette.execute_script('Services.prefs.setBoolPref("dom.sms.enabled", true);')
        result = self.marionette.execute_async_script("return GaiaDataLayer.deleteAllSms();")
        self.marionette.push_permission('sms', False)
        with self.marionette.using_context('chrome'):
            self.marionette.execute_script('Services.prefs.clearUserPref("dom.sms.enabled");')
        return result
...
class Accessibility(object):
    ...
    def _run_async_script(self, func, args):
        ...
        result = self.marionette.execute_async_script(
            '%s\nreturn Accessibility.%s.apply(Accessibility, arguments)' %
            (content, func), args)
