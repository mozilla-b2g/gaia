...
class TestCalendar(GaiaImageCompareTestCase):
    ...
    def test_calendar_flick(self):
        ...
        self.invoke_screen_capture()
        self.scroll(calendar._week_view_locator,'up',3,distance=300)
        self.invoke_screen_capture()
        self.scroll(calendar._week_view_locator,'down',3,distance=200)
        self.invoke_screen_capture()
        self.scroll(calendar._week_view_locator,'left',3,distance=100)
        self.invoke_screen_capture()
        self.scroll(calendar._week_view_locator,'right',3,distance=100)
        self.scroll(calendar._week_view_locator,'right',3,distance=100)
        self.invoke_screen_capture()

        calendar.tap_day_display_button()
        self.invoke_screen_capture()
        self.scroll(calendar._day_view_locator,'up',3,distance=300)
        self.invoke_screen_capture()
        self.scroll(calendar._day_view_locator,'down',3,distance=200)
        self.invoke_screen_capture()
        self.scroll(calendar._day_view_locator,'left',3,distance=100)
        self.invoke_screen_capture()
        self.scroll(calendar._day_view_locator,'right',3,distance=100)
        self.scroll(calendar._day_view_locator,'right',3,distance=100)
        self.invoke_screen_capture()
