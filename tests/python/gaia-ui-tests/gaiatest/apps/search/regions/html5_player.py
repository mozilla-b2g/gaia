...
class HTML5Player(PageRegion):
    ...
    def invoke_controls(self):
        ...
        self.wait_for_condition(lambda m: self.controls_visible)
    ...
    def tap_play(self):
        ...
        self.wait_for_condition(lambda m: self.is_playing is True)

    def tap_pause(self):
        self.tap_video_control('playButton')
        self.wait_for_condition(lambda m: self.is_playing is False)

    def tap_mute(self):
        self.tap_video_control('muteButton')
        self.wait_for_condition(lambda m: self.is_muted is True)

    def tap_unmute(self):
        self.tap_video_control('muteButton')
        self.wait_for_condition(lambda m: self.is_muted is False)
