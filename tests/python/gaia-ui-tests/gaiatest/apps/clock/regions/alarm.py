...
class NewAlarm(Clock):
    ...
    _current_minute_locator = (By.CSS_SELECTOR, 'div.value-picker-minutes-wrapper')
    _minute_picker_locator = (By.CSS_SELECTOR, '.value-picker-minutes div:nth-child(%d)')

    _alarm_time_ok_button_locator = (By.CSS_SELECTOR, '#time-picker-buttons .value-selector-confirm')
    ...
    def spin_minute(self):
        minute = self.marionette.find_element(*self._current_minute_locator).get_attribute('aria-valuenow')
        minute_picker = self.marionette.find_element(self._minute_picker_locator[0], self._minute_picker_locator[1] % (int(minute) + 1))
        minute_picker_move_y = minute_picker.size['height'] * 2
        minute_picker_mid_x = minute_picker.size['width'] / 2
        minute_picker_mid_y = minute_picker.size['height'] / 2

        Actions(self.marionette).flick(minute_picker, minute_picker_mid_x, minute_picker_mid_y, minute_picker_mid_x, - (minute_picker_mid_y + minute_picker_move_y) ).perform()

    def confirm_alarm_time(self):
        self.marionette.find_element(*self._alarm_time_ok_button_locator).tap()
