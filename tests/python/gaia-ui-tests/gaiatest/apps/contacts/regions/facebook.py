...
class FacebookLogin(Base):
    ...
    def __init__(self, marionette):
        ...
        # Make use the Facebook developer account instead of the official one
        import time
        time.sleep(1)
        rr = self.marionette.execute_script("""
        var str = arguments[0].getAttribute('data-url');
        str = str.replace('123456', '323630664378726');
        str = str.replace('395559767228801', '323630664378726')
        arguments[0].setAttribute('data-url', str);
        arguments[0].setAttribute('src', str);
        """, [view])
        time.sleep(1)
