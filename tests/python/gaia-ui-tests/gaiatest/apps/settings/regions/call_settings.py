...
class CallSettings(Base):
    ...
    _fd_page_locator = (By.ID, 'call-fdnSettings')
    ...
    _cf_page_locator = (By.ID, 'call-forwarding')
    _call_forwarding_locator = (By.ID, 'menuItem-callForwarding')
    _cf_status_disabled_text_locator = (By.CSS_SELECTOR, '[data-l10n-id = "callForwardingNotForwarding"]')
    _call_forwarding_always_locator = (By.ID, 'li-cfu-desc')
    _cf_always_page_locator = (By.ID, 'call-cf-unconditional-settings')
    _call_forwarding_busy_locator = (By.ID, 'li-cfmb-desc')
    _cf_busy_page_locator = (By.ID, 'call-cf-mobile-busy-settings')
    _call_forwarding_unanswered_locator = (By.ID, 'li-cfnrep-desc')
    _cf_unanswered_locator = (By.ID, 'call-cf-no-reply-settings')
    _call_forwarding_unreachable_locator = (By.ID, 'li-cfnrea-desc')
    _cf_unreachable_locator = (By.ID, 'call-cf-not-reachable-settings')

    _cb_page_locator = (By.ID, 'call-cbSettings')
    _call_barring_locator = (By.ID, 'menuItem-callBarring')
    _cb_status_text_locator = (By.ID, 'baoc-desc')
    ...
    def cs_screen_element(self):
        return self.marionette.find_element(*self._cs_page_locator)
    ...
    @property
    def fd_screen_element(self):
        return self.marionette.find_element(*self._fd_page_locator)

    @property
    def fd_simpin_screen_element(self):
    ...
    @property
    def cf_screen_element(self):
        return self.marionette.find_element(*self._cf_page_locator)

    @property
    def cf_always_screen_element(self):
        return self.marionette.find_element(*self._cf_always_page_locator)

    @property
    def cf_busy_screen_element(self):
        return self.marionette.find_element(*self._cf_busy_page_locator)

    @property
    def cf_unans_screen_element(self):
        return self.marionette.find_element(*self._cf_unanswered_locator)

    @property
    def cf_unreach_screen_element(self):
        return self.marionette.find_element(*self._cf_unreachable_locator)

    @property
    def cb_screen_element(self):
        return self.marionette.find_element(*self._cb_page_locator)

    @property
    def cb_passcode_screen_element(self):
    ...
    def tap_sim_1(self):
        ...
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cs_page_locator))
    ...
    def tap_caller_id_selection(self):
        ...
        Wait(self.marionette).until(expected.element_displayed(*self._caller_id_confirm_button_locator))
    ...
    def tap_fixed_dialing(self):
        ...
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._fd_page_locator))

    def enable_fixed_dialing(self):
        element = self.marionette.find_element(*self._toggle_fixed_dialing_number_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._sim_pin_area_locator))

    def tap_auth_numbers(self):
        element = self.marionette.find_element(*self._auth_number_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._auth_number_page_locator))

    def tap_call_forwarding(self):
        element = self.marionette.find_element(*self._call_forwarding_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cf_page_locator))

    # the default is 'disabled'
    def wait_until_cf_info_received(self):
        Wait(self.marionette,timeout=20).until(expected.element_displayed(*self._cf_status_disabled_text_locator))

    def tap_always_forward(self):
        element = self.marionette.find_element(*self._call_forwarding_always_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cf_always_page_locator))

    def tap_forward_when_busy(self):
        element = self.marionette.find_element(*self._call_forwarding_busy_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cf_busy_page_locator))

    def tap_forward_unanswered(self):
        element = self.marionette.find_element(*self._call_forwarding_unanswered_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cf_unanswered_locator))

    def tap_forward_unreachable(self):
        element = self.marionette.find_element(*self._call_forwarding_unreachable_locator)
        Wait(self.marionette).until(expected.element_displayed(element) and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cf_unreachable_locator))

    def tap_call_barring(self):
        element = self.marionette.find_element(*self._call_barring_locator)
        Wait(self.marionette).until(expected.element_displayed(element)and expected.element_enabled(element))
        element.tap()
        Wait(self.marionette).until(expected.element_displayed(*self._cb_page_locator))

    def wait_until_cb_info_received(self):
        status = self.marionette.find_element(*self._cb_status_text_locator)
        Wait(self.marionette).until(lambda m: status.get_attribute('data-l10n-id') == 'disabled')
