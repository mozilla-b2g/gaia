...
/**
 * Checks which ref-prebuilt binary we need. This will be null unless
 *
 * 1. Platform is linux
 * 2. gcc not installed
...
function getRequiredVersion(callback) {
  if (process.platform !== 'linux') {
    console.log('[ref-prebuilt] platform not linux');
    return callback(null);
  }

  exec('type g++', function(err, stdout) {
    if (stdout.indexOf('g++ is') !== -1) {
      console.log('[ref-prebuilt] found g++');
      return callback(null);
    }

    exec('lsb_release -a', function(err, stdout) {
      if (stdout.indexOf('precise') === -1) {
        console.log('[ref-prebuilt] distribution not precise');
        return callback(null);
      }
...
function main() {
  getRequiredVersion(function(version) {
    ...
    console.log('[ref-prebuilt] will use existing binary');
    console.log('[ref-prebuilt] ' + version);
