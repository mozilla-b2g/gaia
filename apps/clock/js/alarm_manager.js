 ...
 * we prepare an osAlarms object that contains each alarm type:
 * 'snooze' and 'normal'
 *
...
var AlarmManager = {
  ...
  updateAlarmStatusBar: function am_updateAlarmStatusBar() {
    ...
    request.onsuccess = function(e) {
      ...
      var generator = Utils.asyncGenerator(function(err) {
        if (!err) {
      ...
      for (var i = 0; i < e.target.result.length && !hasAlarmEnabled; i++) {
        AlarmsDB.getAlarm(e.target.result[i].data.id,
          ...
          return function(err, alarm) {
            if (!err) {
              for (var j in alarm.osAlarms) {
                if (alarm.osAlarms[j] === mozAlarm.id) {
