...
contacts.List = (function() {
  ...
  var getSearchString = function getSearchString(contact, display) {
    ...
    if (contact.tel && contact.tel.length) {
      for (i = contact.tel.length - 1; i >= 0; i--) {
        current = contact.tel[i];
        searchInfo.push(current.value);
      }
    }
    if (contact.email && contact.email.length) {
      for (i = contact.email.length - 1; i >= 0; i--) {
        current = contact.email[i];
        searchInfo.push(current.value);
      }
    }
  ...
  var getFilteredContacts = function cl_getFilteredContacts(errorCb,
    ...
    var successCb = successCbk || loadChunk;
      var num = 0;
      var chunk = [];
      var request = navigator.mozContacts.find(options);
  ...
  */
  var exitSelectMode = function exitSelectMode(canceling) {
    ...
    clearClickHandlers();
    if (ActivityHandler.activityName === 'pick' && canceling === true) {
        ActivityHandler.postCancel();
        return;
    }
