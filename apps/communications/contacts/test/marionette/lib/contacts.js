...
Contacts.prototype = {
  ...
  enterContactDetails: function(details) {
    ...
    for (var i in details) {
      ...
      fieldSelector = 'form' + i.charAt(0).toUpperCase() + i.slice(1);

      field = this.client.helper.waitForElement(selectors[fieldSelector]);
      field.sendKeys(details[i]);
    }

    var save = this.client.helper.waitForElement(selectors.formSave),
        title = this.client.helper.waitForElement(selectors.formTitle);
    var isSaveEnabled = function() {
      field.click();
      title.click();
      return save.enabled();
    };
    this.client.waitFor(isSaveEnabled);
    save.click();
