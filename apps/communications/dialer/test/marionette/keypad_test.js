...
var Contacts = require('../../../contacts/test/marionette/lib/contacts.js');
var ContactsData =
  require('../../../contacts/test/marionette/lib/contacts_data');
var ReflowHelper =
...
marionette('Dialer > Keypad', function() {
  ...
  // appears depends on the integration environment.
    ...
    test('Entering a digit in the middle of a long number', function() {
      ...
      var expectedValueAfterInsertion =
        '\u2026' + numberAfterInsertion.substr(4); //1forTheOverFlowingChar,
                                                   //2for the ellipsis itself,
                                                   //1more for the new char.
  ...
  suite('Add number to new or exisiting contact', function() {
    setup(function(){
      dialerApp = new Dialer(client);
    });

    suite('Add number to existing contact', function(){
      var data = [['ZZ', '11111']];
      ...
      setup(function() {
        ...
        test('Add the dialed number to an existing contact', function() {
          var contactSecondPhone = '67890';
          var contactName = 'ZZ';
          dialerApp.launch();
          dialerApp.keypadView.typePhoneNumber(contactSecondPhone);
          var addToContactsActivities = dialerApp.keypadView.addToContacts();
          var contact = addToContactsActivities.pickAddToContact();
          var dialer =
            contact.addToContactAndReturnToDialer(contactName);
          var contactList = dialer.tabs.goToContactList();
          var contactUpdatedName = contactList.getContactFirstName();
          var contactUpdatedPhone =
            contactList.getContactSecondPhoneNumber(contactName);

          assert.strictEqual(contactUpdatedName, contactName);
          assert.strictEqual(contactUpdatedPhone, contactSecondPhone);
      });
    });

    test('Add a new contact from dialer with the dialed number', function() {
      dialerApp.launch();
      dialerApp.keypadView.typePhoneNumber(12345);
      var addToContactsActivities = dialerApp.keypadView.addToContacts();
