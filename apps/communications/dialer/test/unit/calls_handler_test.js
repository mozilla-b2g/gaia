...
require('/dialer/test/unit/mock_conference_group_handler.js');
require('/dialer/test/unit/mock_conference_group_ui.js');
...
suite('calls handler', function() {
  ...
  suiteSetup(function(done) {
    ...
    require('/dialer/js/calls_handler.js', () => {
      subject = CallsHandler;
      done();
    });
  });
  ...
  suite('> headphone and bluetooth support', function() {
    ...
    suite('> bluetooth commands', function() {
      ...
      function triggerCommand(string) {
        /* This should be window.postMessage() but since unit-testing that is a
         * nightmare we call the handler directly. */
        subject.handleCommand({
          origin: COMMS_APP_ORIGIN,
          data: { type: 'BT', command: string }
        });
      }
      ...
      suite('> CHUP', function() {
        test('should end the active call', function() {
          this.sinon.spy(call1, 'hangUp');
          MockNavigatorMozTelephony.calls = [call1];
          MockNavigatorMozTelephony.active = call1;

          triggerCommand('CHUP');
          sinon.assert.calledOnce(call1.hangUp);
        });
      });

      suite('> ATA', function() {
        test('should answer the incoming call', function() {
          this.sinon.spy(call1, 'answer');
          MockNavigatorMozTelephony.calls = [call1];
          MockNavigatorMozTelephony.mTriggerCallsChanged();

          triggerCommand('ATA');
          sinon.assert.calledOnce(call1.answer);
        });
      });

      suite('> CHLD=0', function() {
        test('should hang up the waiting call', function() {
          this.sinon.spy(call2, 'hangUp');
          MockNavigatorMozTelephony.calls = [call1, call2];
          MockNavigatorMozTelephony.mTriggerCallsChanged();

          triggerCommand('CHLD=0');
          sinon.assert.calledOnce(call2.hangUp);
        });
      });
      ...
      suite('> CHLD=3 conference call', function() {
        ...
        function() {
          ...
          triggerCHLD();
          sinon.assert.calledWith(
            MockNavigatorMozTelephony.conferenceGroup.add, call1, call2
          );
        });
        ...
        function() {
          ...
          triggerCHLD();
          sinon.assert.calledWith(
            MockNavigatorMozTelephony.conferenceGroup.add, call1
          );
        });
      });
    });
    ...
    suite('> headset commands', function() {
      ...
      function triggerCommand(string) {
        ...
        subject.handleCommand({
          origin: COMMS_APP_ORIGIN,
          data: { type: 'HS', command: string }
        });
      }

      function triggerHeadset(clock) {
        triggerCommand('headset-button-press');
        clock.tick(10);
        triggerCommand('headset-button-release');
      }
