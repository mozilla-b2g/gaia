...
# For memory consumption reasons and to increase lookup speed, we only store
# a specific number of words in every node and prune the rest.
MaxWordsPerNode = 3
...
def add(word, freq, flags):
    ...
    newLimit = 1;
    while newLimit < PrefixLimit:
        prefix = word[0:min(len(word), newLimit)]
...
# properly. 7% is not really a magic value. Its a good collisition ratio,
