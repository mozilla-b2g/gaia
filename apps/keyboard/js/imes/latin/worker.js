...
var ProxyUserDictPredictor = function(){
  ...
  return {
    ...
    predict: function(...args) {
      return activePredictor.predict.apply(activePredictor, args);
    }
  };
};
    ...
    .then(suggestions2D => {
      ...
      var suggestions =
        [].concat(suggestions2D[0]).concat(suggestions2D[1]).sort(
          (suggestionA, suggestionB) => suggestionB[1] - suggestionA[1]
        );
      ...
      suggestions = suggestions.reduce((prev, curr, index) => {
        var word = curr[0];
