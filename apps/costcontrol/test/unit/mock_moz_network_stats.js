...
var MockMozNetworkStats = (function() {
  ...
  var alarms1 = [{
    'alarmId' : 1,
    'network' : {'type' : 1, 'id' : fakeAllInterfaces[1] }
  }];

  var alarms2 = [{
    'alarmId' : 99,
    'network' : {'type' : 1, 'id' : fakeAllInterfaces[2] }
  }];
  ...
  return {
    ...
    addAlarm: function(networkInterface, limitValue) {
      var result = { result: {} };
      if (networkInterface === fakeAllInterfaces[3]) {
        result = { error: {} };
      }
      return new MockFakeRequest(result);
    },
    getAllAlarms: function(networkInterface) {
      if (networkInterface === fakeAllInterfaces[2]) {
        return new MockFakeRequest({
          result: alarms2
        });
      }
