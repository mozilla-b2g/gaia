...
(function(exports) {
   ...
   * @requires CustomDialog
   */
  function SoundManager() {
    this.overlay = document.getElementById('system-overlay');
    this.notification = document.getElementById('volume');
  ...
  /**
   * A DOMElement for the sound volume overlay.
   *
   * @memberOf SoundManager.prototype
   * @type {DOMElement}
   */
  SoundManager.prototype.overlay = null;
  /**
   * A DOMElement for the sound volume UI.
   *
   * @memberOf SoundManager.prototype
   * @type {DOMElement}
   */
  SoundManager.prototype.notification = null;
   ...
   * @memberOf SoundManager.prototype
   * @return {String} the volume channel
   */
   ...
   * @return {Number} the volume value. (-1) is for slient mode and (0) is for
   *                   vibration mode
   */
  ...
  SoundManager.prototype.changeVolume = function sm_changeVolume(delta,
    ...
    var overlayClasses = this.overlay.classList;
    var notificationClasses = this.notification.classList;

    switch (this.muteState) {
      case 'OFF':
        notificationClasses.remove('mute');
        break;
      case 'MUTE':
        notificationClasses.add('mute');
        break;
    }

    if (this.vibrationEnabled) {
      notificationClasses.add('vibration');
    } else {
      notificationClasses.remove('vibration');
    }
    ...
    var steps =
      Array.prototype.slice.call(this.notification.querySelectorAll('div'), 0);
    ...
    notificationClasses.add('visible');
    ...
    this.activeTimerID = window.setTimeout(function hideSound() {
      ...
      notificationClasses.remove('visible');
    }, 1500);
    ...
    this.notification.dataset.channel = channel;
