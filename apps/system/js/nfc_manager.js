/* globals MozActivity, System, NfcUtils, NDEF, BaseModule */
'use strict';

(function(exports) {
  var NfcManager = function(nfc) {
    this.nfc = nfc;
  };
  NfcManager.prototype = Object.create(BaseModule.prototype);
  NfcManager.prototype.constuctor = NfcManager;
  NfcManager.EVENTS = [
  ...
  var prototype = {
    ...
    init: function nm_init() {
      this.debug('Initializing NFC Message');
    ...
    isScreenUnlockAndEnabled: function nm_isScreenUnlockAndEnabled() {
      // Policy:
      if (System.screenOn && !System.locked) {
        return true;
      } else {
        return false;
      }
    ...
    changeHardwareState: function nm_changeHardwareState(state) {
      this.debug('changeHardwareState - state : ' + state);
      this.hwState = state;
      var nfcdom = this.nfc;
      if (!nfcdom) {
        return;
      ...
      req.onsuccess = function() {
        self.debug('changeHardwareState ' + state + ' success');
      };
      req.onerror = function() {
        self.debug('changeHardwareState ' +
          state + ' error ' + req.error.name);
      };
    },

    _handle_screenchange: function() {
    ...
    '_handle_lockscreen-appclosed': function() {
      this._handle_screenchange();
    },

    '_handle_shrinking-sent': function() {
      window.removeEventListener('shrinking-sent', this);
      // Notify lower layers that User has acknowledged to send nfc (NDEF) msg
      this.publish('dispatch-p2p-user-response-on-active-app', this, true);
      // Stop the P2P UI
      this.publish('shrinking-stop', this, true);
    },
    ...
    fireNDEFDiscovered: function nm_fireNDEFDiscovered(msg, tech) {
      this.debug('fireNDEFDiscovered: ' + JSON.stringify(msg));
      ...
      activity.onerror = () => {
        this.debug('Firing nfc-ndef-discovered failed');
      };
    },
      ...
      function nm_checkP2PRegistration(manifestURL) {
        ...
        status.onsuccess = function() {
          if (status.result) {
            ...
            self.debug('Error checking P2P Registration: ' +
    ...
    fireTagDiscovered: function nm_fireTagDiscovered(msg, type) {
      ...
      a.onerror = function() {
        self.debug('Firing nfc-tag-discovered failed');
      };
    },
    ...
    getPrioritizedTech: function nm_getPrioritizedTech(techList) {
      ...
      techList.sort(function sorter(techA, techB) {
        var priorityA =
          self.TechPriority[techA] || self.TechPriority.Unsupported;
        var priorityB =
          self.TechPriority[techB] || self.TechPriority.Unsupported;
    ...
    handleTechnologyDiscovered: function nm_handleTechnologyDiscovered(msg) {
      ...
      if (window.nfcHandoverManager.tryHandover(
          msg.records, msg.sessionToken)) {
        return;
      }
      ...
      switch (tech) {
        case 'P2P':
          if (!msg.records.length) {
            ...
            // if there are records in the message we've got
            // NDEF messages shared by other device via P2P,
            // this should be handled as regular NDEF
        ...
        case 'NDEF_FORMATABLE':
          ...
          break;
        default:
          this.debug('Tag tech: ' + tech + ', fire Tag-Discovered.');
          this.fireTagDiscovered(msg, tech);
          break;
      }
    },
    ...
    handleTechLost: function nm_handleTechLost(command) {
      this.debug('Technology Lost: ' + JSON.stringify(command));
  ...
  BaseModule.mixin(NfcManager.prototype, prototype);
