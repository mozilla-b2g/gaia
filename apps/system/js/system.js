...
(function(exports) {
  ...
  exports.System = {
    ...
    _stateMachinesByState: new Map(),

    registerState: function(state, provider) {
      this._stateMachines.set(provider.name, provider);
      this._stateMachinesByState.set(state, provider);
    },

    unregisterState: function(state, provider) {
      this._stateMachines.delete(provider.name);
      var machine = this._stateMachinesByState.get(state);
      if (machine === provider) {
        this._stateMachinesByState.delete(state);
      }
    },
    ...
    query: function(stateString) {
      ...
      var state, provider;
      if (args.length > 1) {
        provider = this._stateMachine.get(args[0]);
        state = args[1];
      } else {
        state = args[0];
        provider = this._stateMachinesByState.get(state);
      }
      if (!provider) {
