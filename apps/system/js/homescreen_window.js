(function(window) {
  ...
  var HomescreenWindow = function HomescreenWindow(manifestURL) {
    ...
    // XXX: refine this.
    var config = this.retrieve(manifestURL).origin;
    ...
    this.config.url = this.origin + '/index.html#root';
  ...
  HomescreenWindow.prototype.ensure = function hw_ensure(reset) {
    if (!this.element) {
      ...
      this.browser.element.src = this.config.url + new Date();
    }
    ...
    SettingsListener.observe('homescreen.manifestURL', '',
      function onRetrievingHomescreenManifestURL(value) {
        ...
        if (typeof(instance) !== 'undefined') {
          ...
          self.origin = self.prototype.retrieve(value).origin;
        }
  ...
  HomescreenWindow.getSingletonInstance = function() {
    ...
    if (typeof instance == 'undefined') {
      return new this(currentManifestURL);
