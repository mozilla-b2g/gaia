...
(function(exports) {
  ...
  var EdgeSwipeDetector = function(sheetTransitionManager) {
    this.previous = document.getElementById('left-panel');
    this.next = document.getElementById('right-panel');
    this.screen = document.getElementById('screen');
    this.sheetTransitionManager = sheetTransitionManager;
    this.stackManager = sheetTransitionManager.stack;
    this.init();
  };
  ...
  EdgeSwipeDetector.prototype = {
    ...
    _touchStart: function esd_touchStart(e) {
      if (!this.stackManager.getCurrent()) {
        return;
      }
      this._winWidth = self.layoutManager.width;
      this._direction = (e.target == this.next) ? 'rtl' : 'ltr';
      this._touchStartEvt = e;
      this._startDate = Date.now();

      var iframe = this.stackManager.getCurrent().iframe;
      ...
      this.sheetTransitionManager.begin(this._direction);
    },
    ...
    _touchMove: function esd_touchMove(e) {
      ...
      this.sheetTransitionManager.moveInDirection(
        this._direction, this._progress, this._deltaX);
    },
    ...
    _touchEnd: function esd_touchEnd(e) {
      ...
      if (adjustedProgress < kEdgeThreshold || this._forwarding) {
        this.sheetTransitionManager.snapBack();
        return;
      } else {
        this.sheetTransitionManager.snapForward(speed);
      }

      var direction = this._direction;
      if (direction == 'ltr') {
        this.stackManager.goPrev();
      } else {
        this.stackManager.goNext();
      }
    },
