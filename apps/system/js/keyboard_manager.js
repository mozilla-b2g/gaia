...
(function(exports) {
  ...
  var KeyboardManager = function(inputMethod) {
  ...
  KeyboardManager.EVENTS = [
    ...
    'attentionscreenshow',
    'mozbrowsererror',
    'applicationsetupdialogshow',
    'mozmemorypressure',
    'sheetstransitionstart',
    'lockscreen-appopened',
    'mozChromeEvent'
  ];
  KeyboardManager.SETTINGS = [
    'keyboard.3rd-party-app.enabled'
  ];
  KeyboardManager.SUB_MODULES = [
    'InputAppsTransitionManager',
    'InputFrameManager',
    'ImeSwitcher'
  ];
  System.create(KeyboardManager, {}, {
    name: 'KeyboardManager',
    EVENT_PREFIX: 'keyboard',
    inputTypeTable: {},
    ...
    '_observer_keyboard.3rd-party-app.enabled': function(value) {
    ...
    _handle_applicationsetupdialogshow: function() {
      this.hideKeyboardImmediately();
    },
    _handle_activityrequesting: function() {
      this.hideKeyboardImmediately();
    },
    _handle_activityopening: function() {
      this.hideKeyboardImmediately();
    },
    _handle_activityclosing: function() {
      this.hideKeyboardImmediately();
    },
    ...
    '_handle_mozbrowsererror': function(evt) {
      this.removeKeyboard(evt.target.dataset.frameManifestURL, true);
    },

    _handle_mozmemorypressure: function(evt) {
    ...
    _handle_sheettransitionstart: function() {
      if (this.hasActiveKeyboard) {
        ...
        this.inputMethod.removeFocus();
      }
    },

    '_handle_lockscreen-appopened': function() {
      if (this.hasActiveKeyboard) {
        // Instead of hideKeyboard(), we should removeFocus() here.
        // (and, removing the focus cause Gecko to ask us to hideKeyboard())
        this.inputMethod.removeFocus();
    ...
    getHeight: function kn_getHeight() {
      return this.inputAppsTransitionManager.occupyingHeight;
    },
    ...
    resizeKeyboard: function km_resizeKeyboard(evt) {
      ...
      // while keyboard Frame is transitioning out.
      var transitionState = this.inputAppsTransitionManager.currentState;
      if (transitionState ===
          this.inputAppsTransitionManager.STATE_TRANSITION_OUT) {
        return;
      }

      var height = evt.detail.height;

      this.debug('resizeKeyboard: ' + height);
      this.showingLayoutInfo.height = height;
      this.inputAppsTransitionManager.handleResize(height);
    ...
    inputFocusChange: function km_inputFocusChange(evt) {
      ...
      function activateKeyboard() {
        ...
        if (previousLayout &&
          ...
          self.debug('reset previousFrame.');
      ...
      if (type === 'blur') {
        ...
        self.debug('get focus event ' + type);
    ...
    setKeyboardToShow: function km_setKeyboardToShow(group, index, launchOnly) {
      ...
      this.debug('set layout to display: type=' + group + ' index=' + index);
      ...
      // By setting launchOnly to true,
      // we load the keyboard frame w/o bringing it
      ...
      if (this.inputAppsTransitionManager.currentState ===
          this.inputAppsTransitionManager.STATE_TRANSITION_OUT) {
        this.inputAppsTransitionManager.handleResize(
          this.showingLayoutInfo.height);
      }
    ...
    resetShowingKeyboard: function km_resetShowingKeyboard() {
      this.debug('resetShowingKeyboard');
    ...
    hideKeyboard: function km_hideKeyboard() {
      ...
      var transitionState = this.inputAppsTransitionManager.currentState;
      if ((transitionState === this.inputAppsTransitionManager.STATE_HIDDEN) ||
          (transitionState ===
           this.inputAppsTransitionManager.STATE_TRANSITION_OUT)) {
      ...
      this.inputAppsTransitionManager.hide();
    },

    hideKeyboardImmediately: function km_hideImmediately() {
      this.inputAppsTransitionManager.hideImmediately();
    },
    ...
    setShowingLayoutInfo: function(type, index, layout) {
    ...
    view: function() {
      return '<div id="keyboards" class="hide" data-z-index-level="keyboards">' +
      '<div>';
    }
  });
