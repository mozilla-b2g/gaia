...
var CardsView = (function() {
  ...
  function alignCurrentCard() {
    // We're going to release memory
    deltaX < 0 ? nextCard.dispatchEvent(outViewPortEvent) :
                 prevCard.dispatchEvent(outViewPortEvent);
  ...
  function moveCards() {
    ...
    card.MozTransform = 'scale(' + (SC_SCALE + scaleFactor) + ') translateX(' +
        (translateSign * (100 - (Math.abs(deltaX) * 100) / windowWidth)) + '%)';

    // Fading in new card
    card.opacity = SC_OPA + ((Math.abs(deltaX) / windowWidth) * (1 - SC_OPA));
    // Hiding the old card progressively
    oppositeCard.opacity = SC_OPA - (Math.abs(deltaX) / windowWidth);

    // Fading out, scaling and translating current card
    currentCardStyle.opacity = 1 - ((Math.abs(deltaX) / windowWidth) *
                                    (1 - SC_OPA));
  ...
  function onMoveEvent(evt) {
    ...
    var onReorderingMode = isReorderingMode();

    if (!switchingCard && !onReorderingMode && MANUAL_CLOSING &&
      ...
      if (differenceY > moveCardThreshold) {
        ...
        draggingCardUp = true;
        evt.target.style.MozTransform = 'scale(0.8) translateY(-' +
                                        differenceY + 'px)';
      }
    }

    deltaX = initialTouchPosition.x - touchPosition.x;

    // If we are not reordering or removing Cards now
    // and Snapping Scrolling is enabled, we want to scroll
    // the CardList
    if (!onReorderingMode && !draggingCardUp) {
      if (!switchingCard && Math.abs(deltaX) > switchingCardThreshold) {
        switchingCard = true;
      }
