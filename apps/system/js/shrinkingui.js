...
(function(exports) {
    ...
    (function su_handleEvent(evt) {
      switch (evt.type) {
        case 'appcreated':
          ...
          break;
        case 'appterminated':
          if (this._state() && evt.detail.manifestURL === this.current.appURL)
            this._cleanEffects();
  ...
  ShrinkingUI._receivingEffects =
    (function su_receivingEffects() {
      ...
      var originalVisibility = this.current.iframe.style.visibility;
      this.current.iframe.style.visibility = 'hidden';

      var afterTop = (function() {
        // Restore the display to let it fly in.
        this.current.iframe.style.visibility = originalVisibility;
      ...
      var afterTilt = (function() {
        ...
        // callback won't work).
        this.current.iframe.style.transition = 'transform 0.02s ease';
  ...
  ShrinkingUI._shrinkingTilt =
    (function su_shrinkingTilt(cb) {
      ...
      if (null === this.current.cover) {
        ...
        this.current.wrapper.insertBefore(this.current.cover,
          this.current.iframe);
      }
    ...
    (function su_shrinkingTiltBack(instant) {
        ...
        if (!instant) {
          var tsEnd = (function _tsEnd(evt) {
              this.current.iframe.removeEventListener('transitionend', tsEnd);
          }).bind(this);
          this.current.iframe.style.transition = 'transform 0.3s ease';
          this.current.iframe.addEventListener('transitionend', tsEnd);
  ...
  /**
   * To call the cleaner function.
   *
   * @param {event} |evt| The 'touchend' event.
   * @this {ShrinkingUI}
   */
  ShrinkingUI._handleCleanEffects =
    (function su_handleCleanEffects(evt) {
