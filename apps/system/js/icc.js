  ...
  System.create(Icc, {}, {
      ...
      '<div id="icc-view" class="generic-dialog" data-z-index-level="icc-view" role="dialog">' +
        ...
        '<section role="region" class="skin-organic" id="icc-asyncconfirm">' +
          '<header>' +
            ...
            '<h2 id="icc-asyncconfirm-subtitle"></h2>' +
          '</header>' +
    ...
    getIccInfo: function icc_getIccInfo() {
      ...
      xhr.onerror = function() {
        self.debug('Failed to fetch file: ' + href, xhr.statusText);
      };
      xhr.onload = function() {
        self._defaultURL = xhr.response.defaultURL;
        self.debug('ICC default URL: ', self._defaultURL);
      };
    ...
    getSIMNumber: function icc_getSIMNumber(iccId) {
      self.debug('ICC Getting SIM Number for ' + iccId);
      for (var i = 0; i < window.navigator.mozMobileConnections.length; i++) {
        if (window.navigator.mozMobileConnections[i].iccId === iccId) {
          return i + 1;
        }
    ...
    handleSTKCommand: function icc_handleSTKCommand(message) {
      ...
      if (!message || !message.iccId || !message.command ||
        ...
        return self.debug('STK Proactive Command bad formed: ', message);
      }

      self.debug('STK Proactive Command for SIM ' + message.iccId + ': ',
        message.command);
      if (System.runningFTU) {
        // Delay the stk command until FTU is done
        var self = this;
        window.addEventListener('ftudone', function ftudone() {
          self.debug('FTU is done!... processing STK message:', message);
          self.handleSTKCommand(message);
        });
        return self.debug('FTU is running, delaying STK...');
      }
      ...
      self.debug('STK Command not recognized ! - ', message);
    },
    ...
    responseSTKCommand: function icc_responseSTKCommand(message, response) {
      self.debug('STK sendStkResponse -- # response = ', response);
    ...
    terminateResponse: function(message) {
      self.debug('STK Sending STK_RESULT_UICC_SESSION_TERM_BY_USER to card ' +
        message.iccId);
    ...
    backResponse: function(message) {
      self.debug('STK Sending STK_RESULT_BACKWARD_MOVE_BY_USER to card ' +
        message.iccId);
      this.responseSTKCommand(message, {
    ...
    showURL: function(stkMessage, url, confirmMessage) {
      ...
      self.debug('Final URL to open: ' + url);
      if (url != null || url.length != 0) {
        if (confirmMessage) {
    ...
    input: function(stkMessage, message, timeout, options, callback) {
      ...
      function clearInputTimeout() {
        if (timeoutId) {
          self.debug('clearing previous STK INPUT timeout');
          clearTimeout(timeoutId);
          timeoutId = null;
        }
      }
      function setInputTimeout() {
        self.debug('setting new STK INPUT timeout to - ', timeout);
        if (timeout) {
          clearInputTimeout();
