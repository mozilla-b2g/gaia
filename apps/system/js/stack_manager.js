...
(function(exports) {
  ...
  StackManager.prototype = {
    ...
    getPrev: function sm_getPrev() {
      var inGroupPrev = this.getCurrent().getPrev();
      if (inGroupPrev) {
        return inGroupPrev;
      }
    ...
    getNext: function sm_getNext() {
      var inGroupNext = this.getCurrent().getNext();
      if (inGroupNext) {
        return inGroupNext;
      }
    ...
    goPrev: function sm_goPrev() {
      ...
      if (newApp.groupID !== oldApp.groupID) {
        this.position--;
        AppWindowManager._updateActiveApp(newApp.instanceID);
      }
    },
