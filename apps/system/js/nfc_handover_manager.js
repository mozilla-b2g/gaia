/* globals NDEFUtils, NfcConnectSystemDialog,
           NDEF, NfcUtils, NotificationHelper, System, BaseModule */
...
(function(exports) {
   ...
   */
  var NfcHandoverManager = function(bluetoothTransfer) {
    this.bluetoothTransfer = bluetoothTransfer;
    this.bluetooth = System.API.bluetooth;
    this.nfc = System.API.nfc;
    this.settings = System.API.settings;
  };
  NfcHandoverManager.prototype = Object.create(BaseModule.prototype);
  NfcHandoverManager.prototype.constructor = NfcHandoverManager;
  NfcHandoverManager.SYSTEM_MESSAGES = ['nfc-manager-send-file'];
  NfcHandoverManager.EVENTS = ['bluetooth-adapter-added'];
  var prototype = {
    ...
    /**
     * Set whenever a timeout is defined
     * while waiting for an outstanding handover response.
     * @type {Object}
     * @memberof NfcHandoverManager.prototype
     */
    ...
    '_handle_bluetooth-adapter-added': function(evt) {
      this.debug('bluetooth-adapter-added');
    ...
    '_watch_nfc-manager-send-file': function(msg) {
      this.debug('In New event nfc-manager-send-file' + JSON.stringify(msg));
      this.handleFileTransfer(msg.sessionToken, msg.blob, msg.requestId);
    },
    ...
    _restoreBluetoothStatus: function _restoreBluetoothStatus() {
      if (!this.isHandoverInProgress() &&
          this.bluetoothTransfer.isSendFileQueueEmpty) {
    ...
    /**
     * This function will be called after a timeout
     * when we did not receive the Hs record within three seconds.
     * At this point we cancel the file transfer.
     * @memberof NfcHandoverManager.prototype
     */
    ...
    _doFileTransfer: function _doFileTransfer(mac) {
      ...
      this.bluetoothTransfer.sendFileViaHandover(mac, blob);
    },
    ...
    _doHandoverRequest: function _doHandoverRequest(ndef, session) {
      ...
      if (nfcPeer === null) {
        this.debug('NFC peer went away during doHandoverRequest');
        this._showFailedNotification('transferFinished-receivedFailed-title');
        this._restoreBluetoothStatus();
        return;
      }
    ...
    _initiateFileTransfer:
      function _initiateFileTransfer(session, blob, requestId) {
        ...
        var nfcPeer = this.nfc.getNFCPeer(session);
        if (nfcPeer === null) {
          this.debug('NFC peer went away during initiateFileTransfer');
          onerror();
          this._restoreBluetoothStatus();
          this._showFailedNotification('transferFinished-sentFailed-title',
                                       blob.name);
     ...
     * @memberof NfcHandoverManager.prototype
     */
    _dispatchSendFileStatus: function(status, requestId) {
  ...
  BaseModule.mixin(NfcHandoverManager.prototype, prototype);
