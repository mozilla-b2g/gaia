...
(function(exports) {
  ...
  var TrustedUIManager = function() {};
  System.create(TrustedUIManager, {
    get currentStack() {
    ...
    _hideCallerApp: function trui_hideCallerApp(origin, callback) {
      var app = System.getRunningApp(origin);
    ...
    _restoreCallerApp: function trui_restoreCallerApp(origin) {
      ...
      var frame = app.element;
      frame.style.visibility = 'visible';
      frame.classList.remove('back');
      if (!System.topMostAppWindow.isHomescreen) {
        this.publish('trusteduihide', { origin: origin });
      }
      if (System.topMostAppWindow.origin == origin) {
    ...
    handleEvent: function trui_handleEvent(evt) {
      switch (evt.type) {
        ...
        case 'appwillclose':
          ...
          break;
        case 'keyboardchange':
          this._setHeight(layoutManager.height - StatusBar.height);
          break;
        case 'keyboardhide':
          this._setHeight(layoutManager.height - StatusBar.height);
          break;
  ...
  exports.TrustedUIManager = TrustedUIManager;
}(window));
