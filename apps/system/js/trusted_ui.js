...
var TrustedUIManager = {
  ...
  _restoreCallerApp: function trui_restoreCallerApp(app) {
    ...
    if (!Service.query('AppWindowManager.getActiveApp').isHomescreen) {
      this.publish('trusteduihide', { origin: app.origin });
    }
    if (Service.query('AppWindowManager.getActiveApp').origin == app.origin) {
  ...
  handleEvent: function trui_handleEvent(evt) {
    switch (evt.type) {
      ...
      case 'appwillclose':
        ...
        break;
      case 'keyboardchange':
        var keyboardHeight = Service.query('InputWindowManager.getHeight');
