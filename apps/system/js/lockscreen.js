...
(function(exports) {
  ...
  */
  ...
  /**
   * the fade-out gradient masks only shows
   * when:
  ...
  function ls_setNotificationMaskArrowVisibility(fromNewNotification) {
    // mask
    ...
    // arrow
    // arrow only shows when there is new notification
    // otherwise it can hide and can't reappear once it's hidden
    if(fromNewNotification){
      // show when user is not at bottom and container is scrollable
      if((this.notificationsContainer.scrollTop +
          this.notificationsContainer.clientHeight <
          this.notificationsContainer.scrollHeight) &&
         (this.notificationsContainer.clientHeight <
