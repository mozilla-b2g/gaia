/* global AppWindow, MockShrinkingUI, HomescreenWindow, MocksHelper,
          MockRocketbar, MockTaskManager, MockFtuLauncher, MockService,
          MockUtilityTray, MockNavigatorSettings, BaseModule */
'use strict';
...
suite('system/AppWindowManager', function() {
  ...
  suite('Settings change', function() {
    ...
    test('nfc.enabled', function() {
      assert.isNull(subject._nfcHandler,
                    '_nfcHandler should be null when NFC disabled');

      // enable NFC to instantiate _nfcHandler before stubbing its methods
      MockNavigatorSettings.mTriggerObservers('nfc.enabled',
        { settingValue: true });
      assert.isNotNull(subject._nfcHandler);

      var stubNfcStart = this.sinon.stub(subject._nfcHandler, 'start');
      var stubNfcStop = this.sinon.stub(subject._nfcHandler, 'stop');
