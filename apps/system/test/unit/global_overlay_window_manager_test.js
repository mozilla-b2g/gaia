  ...
  suite('system/GlobalOverlayWindowManager', function() {
    ...
    suite('system-resize', function() {
      ...
      setup(function() {
        ...
        stubWaitUntil = this.sinon.stub(detail, 'waitUntil');
      });
      ...
      teardown(function() {
        ...
        stubResize.restore();
        stubWaitUntil.restore();
      });

      test('Handle system-resize', function() {
        subject.handleEvent({
          type: 'globaloverlaycreated',
          detail: fakeOverlay
        });
        ...
        assert.ok(stubWaitUntil.calledOnce, 'Should call waitUntil');
        assert.equal(subject._instance, fakeOverlay);
      });
      ...
      test('Handle system-resize - non existing overlay', function() {
        ...
        assert.ok(stubWaitUntil.notCalled, 'Should not call waitUntil');
        assert.isNull(subject._instance);
      });
