...
var mocksForAttentionWindowManager = new MocksHelper([
  ...
  'HomescreenWindow', 'AppWindowManager'
]).init();
...
suite('system/AttentionWindowManager', function() {
  ...
  suite('Handle events', function() {
    ...
    test('broadcast utility tray opening event', function() {
      attentionWindowManager._instances =
        new Map([[att3, att3], [att2, att2], [att1, att1]]);
      var stubBroadcast = [
        this.sinon.stub(att1, 'broadcast'),
      ...
      assert.isTrue(stubBroadcast[0].calledWith('utility-tray-overlayopening'));
      assert.isTrue(stubBroadcast[1].calledWith('utility-tray-overlayopening'));
      assert.isTrue(stubBroadcast[2].calledWith('utility-tray-overlayopening'));
    });
