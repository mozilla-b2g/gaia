...
// Bug 840500.
  ...
  suite('fullscreen mode >', function() {
    ...
    test('the status bar should expand when the Rocketbar expands',
      function() {
      StatusBar.collapse();

      var evt = new CustomEvent('rocketbarexpand');
      StatusBar.handleEvent(evt);

      assert.isTrue(StatusBar.background.classList.contains('expanded'));
    });


    test('the status bar should collapse when the Rocketbar collapses',
      function() {
      StatusBar.expand();

      var evt = new CustomEvent('rocketbarcollapse');
      StatusBar.handleEvent(evt);

      assert.isFalse(StatusBar.background.classList.contains('expanded'));
    });
