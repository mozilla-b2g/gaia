/* globals AttentionWindow, MocksHelper, AppWindow, MockApplications,
            MockL10n, MockLayoutManager, layoutManager */
'use strict';
...
suite('system/AttentionWindow', function() {
  ...
  suite('attention window instance.', function() {
    ...
    suite('set clip', function() {
      test('set clip', function() {
        var attention = new AttentionWindow(fakeAttentionConfig, app);
        this.sinon.stub(attention, 'isActive').returns(false);
        this.sinon.stub(attention, 'isHidden').returns(false);
        attention.setClip(200, 100);
        assert.equal(attention.element.style.clip,
          'rect(0px, ' + layoutManager.width + 'px, 100px, 0px)');
      });

      test('Does not work while attention is active', function() {
        var attention = new AttentionWindow(fakeAttentionConfig, app);
        this.sinon.stub(attention, 'isActive').returns(true);
        this.sinon.stub(attention, 'isHidden').returns(false);
        attention.setClip(200, 100);
        assert.notEqual(attention.element.style.clip,
          'rect(0px, ' + layoutManager.width + 'px, 100px, 0px)');
      });

      test('Does not work while attention is hidden', function() {
        var attention = new AttentionWindow(fakeAttentionConfig, app);
        this.sinon.stub(attention, 'isActive').returns(true);
        this.sinon.stub(attention, 'isHidden').returns(true);
        attention.setClip(200, 100);
        assert.notEqual(attention.element.style.clip,
          'rect(0px, ' + window.layoutManager.width + 'px, 100px, 0px)');
      });
    });

    suite('unset clip', function() {
      test('Remove class', function() {
        var attention = new AttentionWindow(fakeAttentionConfig, app);
        attention.element.classList.add('in-utility-tray');
        attention.unsetClip();
        assert.isFalse(attention.element.classList.contains('in-utility-tray'));
      });
    });
