...
var MimeMapper,
    ...
    prettyDate = require('date').prettyDate;
...
return [
  require('./base')(require('template!./message_reader.html')),
  {
    createdCallback: function() {
      ...
      this._on(this.previousBtn, 'click', 'onPrevious');
      this._on(this.nextBtn, 'click', 'onNext');
      this._on(this.replyBtn, 'click', 'onReplyMenu');
      this._on(this.deleteBtn, 'click', 'onDelete');
      this._on(this.starBtn, 'click', 'onToggleStar');
      this._on(this.moveBtn, 'click', 'onMove');
      this._on(this.envelopeBar, 'click', 'onEnvelopeClick');
      this._on(this.loadBar, 'click', 'onLoadBarClick');
    ...
    _on: function(node, eventName, method) {
    ...
    onHyperlinkClick: function(event, linkNode, linkUrl, linkText) {
      ...
      content.textContent = mozL10n.get('browse-to-url-prompt',
                                        { url: linkUrl });
      ConfirmDialog.show(dialog,
        { // Confirm
          id: 'msg-browse-ok',
    ...
    buildHeaderDom: function(domNode) {
      ...
      function updatePeep(peep) {
        var nameNode = peep.element
                       .getElementsByClassName('msg-peep-content')[0];
      ...
      if (body.attachments && body.attachments.length) {
        ...
        require(['shared/js/mime_mapper'], function(mapper) {
          ...
          var attTemplate = msgAttachmentItemNode,
              filenameTemplate = attTemplate
                          .getElementsByClassName('msg-attachment-filename')[0],
              filesizeTemplate = attTemplate
                          .getElementsByClassName('msg-attachment-filesize')[0];
          for (var iAttach = 0; iAttach < body.attachments.length; iAttach++) {

            // Create an element to hold this attachment.
            var attNode = this.attachmentsContainer.childNodes[iAttach];
            if (!attNode) {
              attNode = this.attachmentsContainer.appendChild(
            ...
            filesizeTemplate.textContent = this.prettyFileSize(
              attachment.sizeEstimateInBytes);

            var attachmentNode = attTemplate.cloneNode(true);
            this.attachmentsContainer.replaceChild(attachmentNode, attNode);
    ...
    disableReply: function() {
      this.replyBtn.setAttribute('aria-disabled', true);
    },

    enableReply: function() {
      this.replyBtn.removeAttribute('aria-disabled');
    },
