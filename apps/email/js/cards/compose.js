...
return [
  require('./base')(require('template!./compose.html')),
  {
    ...
    createdCallback: function() {
      ...
      this.updateAttachmentsSize = this.updateAttachmentsSize.bind(this);

      this.htmlIframeNode = null;

      // Add Contact-add buttons event listener
      var i,
          addBtns = this.getElementsByClassName('cmp-contact-add');
      for (i = 0; i < addBtns.length; i++) {
        addBtns[i].addEventListener('click', this.onContactAdd.bind(this));
      }
      // Add input focus:
      var containerList = this.getElementsByClassName('cmp-combo');
      ...
      var attachmentBtns =
        this.getElementsByClassName('cmp-attachment-btn');
      for (i = 0; i < attachmentBtns.length; i++) {
    ...
    fromEditor: function(value) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (node.nodeName === 'BR' &&
            ...
            // synthetic trailing BR with type="_moz".  We do not like/need this
            // synthetic BR, so we filter it out.  Check out
    ...
    isEmptyAddress: function() {
      ...
      var bubbles = this.addrBar.querySelectorAll('.cmp-peep-bubble');
    ...
    onContainerClick: function(evt) {
      ...
      if (target.classList.contains('cmp-peep-bubble')) {
        ...
        Marquee.setup(email, this.headerNode);
      ...
      if (this.composer.attachments && this.composer.attachments.length) {
        ...
        for (var i = 0; i < this.composer.attachments.length; i++) {
            ...
            ConfirmDialog.show(dialog,
             {
              // ok
              id: 'msg-attach-ok',
              handler: this.updateAttachmentsSize
             }
            );
            return;
          }
          ...
          filesizeTemplate.textContent =
                                      this.prettyFileSize(attachment.blob.size);
    ...
    updateAttachmentsSize: function() {
      ...
      if (this.composer.attachments.length === 0) {
        ...
        this.attachmentsSize.textContent = this.prettyFileSize(totalSize);
      }
