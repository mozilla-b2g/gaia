...
var cards = {
  ...
  pushCard: function(type, showMethod, args, placement) {
    ...
    var domNode = new cardDef(domNode);
    if (domNode.extraAttributes) {
      Object.keys(domNode.extraAttributes).forEach(function(name) {
        domNode.setAttribute(name, domNode.extraAttributes[name]);
      });
    }
  ...
  _findCardUsingType: function(type) {
    for (var i = 0; i < this._cardStack.length; i++) {
      ...
      if (translateCustomElementName(cardInst.cardDef.name) === type) {
        return i;
      }
    }
  },
  ...
  isVisible: function(cardImpl) {
    return !!(cardImpl &&
              cardImpl.classList.contains('center'));
  },
  ...
  getCurrentCardType: function() {
    ...
    if (this._pendingPush) {
      ...
      result = translateCustomElementName(card.cardDef.name);
    }
    return result;
  },
