...
marionette('the gallery', function() {
  ...
  setup(function() {
    ...
    client.fileManager.add({
      type: 'pictures',
      filePath: 'test_media/Pictures/firefoxOS_640x480.jpg'
    });
  ...
  test('should flick through images in fullscreen mode', function() {
    ...
    actions.flick(app.fullscreenFrame2, center.x + 100,
                  center.y, center.x - 100, center.y).perform();
  ...
  test('Zoom image in fullscreen mode', function() {
    app.thumbnail.click();
    var translateX = app.getFrameTranslation(app.fullscreenFrame2);
    assert.strictEqual(translateX, 0);
    ...
    // Check image scale before zoom
    assert.ok(app.checkImgScale(app.frame2Image, scale.normal));

    // Double tap to zoom image
    actions.doubleTap(app.fullscreenFrame2).perform();
    assert.ok(app.checkImgScale(app.frame2Image, scale.zoom));

    //Double tap to return image to actual size
    actions.doubleTap(app.fullscreenFrame2).perform();
    assert.ok(app.checkImgScale(app.frame2Image, scale.normal));
  });
});
