...
(function(exports) {
  ...
  function createThumbnails() {
    // If the db is ready, start enumerating it. Otherwise wait
    // until it becomes ready
    if (photodb.state === MediaDB.READY) {
      enumerateDB();
    }
    else {
      photodb.addEventListener('ready', enumerateDB);
    }
  }

  function enumerateDB() {
    // MediaDB can send a ready event more than once. We only want to
    // enumerate the db the first time, so make sure we only get called once.
    photodb.removeEventListener('ready', enumerateDB);
