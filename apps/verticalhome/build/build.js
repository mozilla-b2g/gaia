...
HomescreenAppBuilder.prototype.bestMatchingIcon =
  function(preferredSize, manifest, origin) {
...
HomescreenAppBuilder.prototype.customizeHomescreen = function() {
  ...
  var searchPageDebug = false;

  let localSettingsFile =
    utils.getFile(config.APP_DIR, 'everything.me', 'config', 'local.json');
  if (localSettingsFile.exists()) {
    let localSettings = utils.getJSON(localSettingsFile);
    searchPageDebug = localSettings.debug;
  }

  let content = {
    search_page: {
      provider: 'EverythingME',
      debug: searchPageDebug,
      separate_page: false,
      enabled: searchPageEnabled
    },

    tap_threshold: tapThreshold,
    tap_effect_delay: tapEffectDelay,
    move_collection_threshold: moveCollectionThreshold,
