...
suite('FindMyDevice >', function() {
  ...
  suite('settings handlers', function() {
    ...
    suite('Changes to findmydevice.registered', function() {
      var contactServerSpy, loadStateStub, currentIdHelperSpy;
      var _registered = {};
      var stubState   = {
        clientid: 'fakeId'
      };
      ...
      setup(function() {
        ...
        loadStateStub       = this.sinon.stub(FindMyDevice, '_loadState',
          function(cb) {
            FindMyDevice._state = stubState;
            cb && cb();
          });
        currentIdHelperSpy  =
      ...
      suite('when not registered', function() {
        ...
        test('contact server', function() {
          ...
          sinon.assert.notCalled(loadStateStub);
      ...
      suite('when registered', function() {
        ...
        test('does load state', function() {
          FindMyDevice._onRegisteredChanged(_registered);
          sinon.assert.calledOnce(loadStateStub);
        });
        ...
        test('does set empty current id', function() {
          ...
          sinon.assert.calledWith(currentIdHelperSpy, 'fakeId');
        });
      });
    });
