...
suite('Contacts', function() {
  ...
  suiteSetup(function() {
    ...
    navigator.mozContacts = {
      ...
      find: function(filter) {
        ...
        return Promise.reject({ name: 'Mock missing filter params' });
      }
    };
  ...
  suite('Contacts.findByPhoneNumber', function() {
    ...
    test('The mozContacts find() call returned an error', function(done) {
      Contacts.findByPhoneNumber('callonerror').then(function(contacts) {
        assert.equal(contacts.length, 0);
      }).then(done, done);
    });
  ...
  suite('Contacts.findByAddress', function() {
    ...
    test('The mozContacts find() call returned an error', function(done) {
      Contacts.findByAddress('callonerror').then(function(contacts) {
        assert.equal(contacts.length, 0);
      }).then(done, done);
    });
