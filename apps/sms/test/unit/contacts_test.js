...
suite('Contacts', function(done) {
  ...
  suite('Contacts.addUnknown', function() {
    ...
    test('For adding an unknown contact', function() {
      ...
      assert.equal(length + 1, Contacts.getLength());
    });
    ...
    test('For not adding the duplicate unknown contact', function() {
      ...
      Contacts.addUnknown('123456');
      assert.equal(length, Contacts.getLength());
    });
  });
  ...
  suite('Contacts.findByUnknown', function() {

    test('For checking the matches for findByUnknown', function() {

      var unknownNumbers = ['12345', '1234569', '123456789', '789789'];
      Contacts.findByUnknown('12345', function() {
        for (var i = 0, length = unknownNumbers.length; i < length; i++) {
          var num = unknownNumbers[i];
          if (num.contains('12345')) {
            if (i === 0) {
              assert.equal(num, '12345');
            } else if (i === 1) {
                assert.equal(num, '1234569');
            } else if (i === 2) {
                assert.equal(num, '123456789');
            } else {
                assert.equal(num, '789789');
            }
          }
        }
     });
