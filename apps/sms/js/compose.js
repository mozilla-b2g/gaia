...
var Compose = (function() {
  ...
  function renderImages() {
    var imgNodes = getImageAttachments();

    return Promise.all(
      imgNodes.map((node) => attachments.get(node).updateThumbnail())
    ).then(() => undefined);
  }

  function handleImagesResize() {
  ...
  var compose = {
    ...
    fromDraft: function(draft) {
      ...
      draft.content.forEach(function(fragment) {
        ...
        Compose.append(fragment, { batch: true });
      }, Compose);
        ...
        SMIL.parse(message, function(elements) {
          elements.forEach(function(element) {
            if (element.blob) {
              ...
              this.append(attachment, { batch: true });
            }
            if (element.text) {
              this.append(element.text, { batch: true });
            }
          }, this);
    ...
    append: function(item, options) {
      ...
      if (Array.isArray(item)) {
        item.forEach((content) => {
          this.append(content, { batch: true });
        });
