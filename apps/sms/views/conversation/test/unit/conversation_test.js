...
suite('conversation.js >', function() {
  ...
  suite('deleting messages and threads', function() {
    ...
    setup(function(done) {
      ...
      this.sinon.stub(MessageManager, 'getMessage').returns(Promise.resolve());

      this.sinon.stub(Threads, 'unregisterMessage', (messageId) => {
        ConversationView.activeThread.messages.delete(messageId);
      });
    ...
    test('error still resolves deleteMessages', function(done) {
      ...
      ConversationView.deleteMessages([]).catch(() => {
  ...
  suite('beforeLeave() ', function() {
    ...
    test('does not update/reset draft if moving to related report/group view',
    function() {
      var existingDraft = { id: 1 };
      ConversationView.draft = existingDraft;
      this.sinon.spy(ConversationView, 'updateDraft');
