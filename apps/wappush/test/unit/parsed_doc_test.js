    ...
    test('Document with same appid different provider id',
      function() {
        ...
        var provisioningDoc =
          ...
          '</wap-provisioningdoc>';
        parsedProvisioningDoc = ParsedProvisioningDoc.from(provisioningDoc);
        var apns = parsedProvisioningDoc.getApns();
        assert.lengthOf(apns, 2);
        assert.equal(apns[0].proxy, '10.0.0.172');
        assert.equal(apns[0].port, '9201');
        assert.equal(apns[0]['TO-NAPID'][0], 'cmccwap_NAPID');
        assert.equal(apns[1].proxy, '10.0.0.172');
        assert.equal(apns[1].port, '9203');
        assert.equal(apns[1]['TO-NAPID'][0], 'cmccwap_NAPID');
    });
  });
});
