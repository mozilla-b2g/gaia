...
AppServer.prototype = {
  ...
  requestHandler: function() {
    ...
    return function(req, res) {
      ...
      var route = routes[pathname] ? pathname : '*';
      var handler = routes[route] || routes[route];
      var settings = this.settings(pathname);
      if (!this._requestsByPathname[pathname]) {
        this._requestsByPathname[pathname] = 0;
      }
      this._requestsByPathname[pathname]++;
      debug('requestHandler', req.method, req.url, 'route:'+route,
        'pathname/count: ', pathname, this._requestsByPathname[pathname] || 0);
...
var routes = {
  ...
  */
  '/settings/set_response': decorateHandlerForJSON(function(req, res) {
    ...
    var settings = this.settings(path);
  ...
  */
  ...
  '/meta/request_count': function(req, res) {
    var query = Url.parse(req.url, true).query;
    var path = query.url && Url.parse(query.url).pathname;
    var count = this._requestsByPathname[path || '*'];
    debug('request_count handler, path/count: ', path, count);
    writeJSON(res, {
      path: path || '*',
      count: count,
      all: this._requestsByPathname
    });
  },
