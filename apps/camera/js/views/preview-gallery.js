...
return View.extend({
  ...
  render: function() {
    ...
    // video player use tap event instead of click event.
    bind(this.el, 'tap', this.onTap);
    bind(this.els.mediaFrame, 'orientationSwipe', this.orientationSwipe);
    bind(this.els.mediaFrame, 'videoPlayerState', this.handleVideoPlayer);
  ...
  onTap: function() {
    var isShwon = this.els.previewMenu.classList.contains('visible');
    if(isShwon) {
