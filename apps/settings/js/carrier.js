...
require([
  ...
  var CarrierSettings = (function() {
    ...
    function cs_init() {
      ...
      window.addEventListener('panelready', function(e) {
        ...
        if (currentHash === '#carrier') {
          ...
          navigator.mozL10n.setAttributes(detailHeader,
        ...
        if (currentHash === '#carrier-operatorSettings') {
          SettingsUtils.getSupportedNetworkInfo(_mobileConnection,
            function(result) {
    ...
    /**
     * Update the network type limited items' visibility
     * based on the voice type.
     */
    function cs_updateNetworkTypeLimitedItemsVisibility(conn) {
      // The following features are limited to GSM types.
      var autoSelectOperatorItem =
        document.getElementById('operator-autoSelect');
      ...
      if (!voiceType) {
        SettingsUtils.getSupportedNetworkInfo(conn, function(result) {
    ...
    function cs_initNetworkTypeText(aNext) {
      ...
      try {
        ...
        req.onerror = function() {
          console.error('Error loading ' + NETWORK_TYPE_SETTING +
            ' settings. ' + req.error && req.error.name);
          aNext && aNext();
        };
      } catch (e) {
        console.error('Error loading ' + NETWORK_TYPE_SETTING +
    ...
    /**
     * Init network type selector. Add the event listener that
     * handles the changes for the network type.
     */
    function cs_initNetworkTypeSelector() {
      ...
      continueButton.addEventListener('click', function onClickHandler() {
        ...
        SettingsUtils.getSupportedNetworkInfo(
          _mobileConnection, cs_updateNetworkTypeSelector);
      });
