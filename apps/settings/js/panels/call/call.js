...
define(function(require) {
  ...
  var TaskScheduler = require('panels/call/task_scheduler');
  ...
  function Call(panel) {
    ...
    this._mobileConnections = window.navigator.mozMobileConnections;
    this._mobileConnection = null;
    ...
    this._getCallForwardingOptionSuccess = true;
    this._voiceServiceClassMask = null;
    this._voiceTypes = [].map.call(this._mobileConnections, function() {
      return null;
    });
  }

  Call.prototype = {
    init: function(cardIndex) {
      this._cardIndex = cardIndex;
      this._mobileConnection = this._mobileConnections[cardIndex];
      if (!this._mobileConnection) {
        return;
      }

      this._voiceServiceClassMask =
        this._mobileConnection.ICC_SERVICE_CLASS_VOICE;

      // add event listeners
      var voiceType =
        this._mobileConnection.voice && this._mobileConnection.voice.type;

      this._addVoiceTypeChangeListeners();
      this._updateNetworkTypeLimitedItemsVisibility(voiceType);
      ...
      // TODO : wait for bug 1103810
      // remember to add cs_initCallForwardingObservers back,
      // I already got another patch fixing this part.

    },

    _addVoiceTypeChangeListeners: function() {
      Array.prototype.forEach.call(_mobileConnections, (conn, index) => {
        this._voiceTypes[index] = conn.voice.type;
        conn.addEventListener('voicechange', function() {
          var newType = conn.voice.type;
          if (index !== this._cardIndex || 
            this._voiceTypes[index] === newType) {
              return;
          }
          this._voiceTypes[index] = newType;
    ...
    _updateNetworkTypeLimitedItemsVisibility: function(voiceType) {
      ...
      this._elements.callForwardingMenuItem.hidden =
      this._elements.callBarringMenuItem.hidden =
      this._elements.callWaitingMenuItem.hidden =
      this._elements.callerIdMenuItem.hidden =
        (CallConstant.NETWORK_TYPE_CATEGORY[voiceType] !== 'gsm');

      // The following feature is limited to CDMA types.
      this._elements.voicePrivacyMenuItem.hidden =
        (CallConstant.NETWORK_TYPE_CATEGORY[voiceType] !== 'cdma');
    },

    _initVoiceMailClickEvent: function() {
      this._elements.voicemailMenuItem.onclick = () => {
        DialogService.show('call-voiceMailSettings');
      };
    },

    _initFdnItm: function() {
      var iccObj = SettingsUtils.getIccByIndex(this._cardIndex);
      iccObj.getServiceState('fdn').then((hasFdn) => {
        this._elements.fdnMenuItem.hidden = !hasFdn;
      });
    },

    _iterateConns: function(callback) {
      var conns = this._mobileConnections;
      [].forEach.call(conns, (conn, index) => {
        callback(conn, index);
      });
    }
  };
  ...
  var Call = (function() {
    function cs_init() {
      ...
      // TODO we are adding event listeners now
    ...
    function cs_getCallForwardingOption(callback) {
      ...
      var unconditional = _mobileConnection.getCallForwardingOption(
        CallConstant.CF_REASON.CALL_FORWARD_REASON_UNCONDITIONAL);
      ...
      unconditional.onsuccess = function() {
        ...
        var mobileBusy = _mobileConnection.getCallForwardingOption(
          CallConstant.CF_REASON.CALL_FORWARD_REASON_MOBILE_BUSY);
        ...
        mobileBusy.onsuccess = function() {
          ...
          var noReply = _mobileConnection.getCallForwardingOption(
            CallConstant.CF_REASON.CALL_FORWARD_REASON_NO_REPLY);
          ...
          noReply.onsuccess = function() {
            ...
            var notReachable = _mobileConnection.getCallForwardingOption(
              CallConstant.CF_REASON.CALL_FORWARD_REASON_NOT_REACHABLE);
            ...
            notReachable.onsuccess = function() {
              ...
              Object.keys(cfOptions).forEach(function(settingKey) {
                ...
                for (var i = 0; i < rules.length; i++) {
                  if (rules[i].active &&
                    ((_voiceServiceClassMask & rules[i].serviceClass) !== 0)) {
                    _cfReasonStates[CallConstant.CF_REASON_MAPPING[settingKey]] = 1;
                ...
                if (!hasValidRule) {
                  _cfReasonStates[CallConstant.CF_REASON_MAPPING[settingKey]] = 0;
                  // Send the latest query result from carrier to system app
                  asyncOpChecker.runTask(
                    cs_setToSettingsDB,
                    'ril.cf.' + settingKey + '.number',
    ...
    function cs_getSetCallForwardingOptionResult(rules, action) {
      ...
      for (var i = 0; i < rules.length; i++) {
        if (rules[i].active &&
            ((_voiceServiceClassMask & rules[i].serviceClass) !== 0)) {
          var disableAction = action === CallConstant.CF_ACTION.CALL_FORWARD_ACTION_DISABLE;
      ...
      var registrationAction =
        action === CallConstant.CF_ACTION.CALL_FORWARD_ACTION_REGISTRATION;
