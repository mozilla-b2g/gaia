...
suite('about >', function() {
  ...
  suiteTeardown(function() {
    ...
    MockNavigatorMozIccManager.addIcc('12345', {
      'cardState': 'ready'
    });
  ...
  suite('checkForUpdates >', function() {
    ...
    suite('getting response for system update >', function() {
      suite('successful >', function() {
        var handlerToBeRemoved = null;
        setup(function() {
          handlerToBeRemoved = geckoUpdateHandlers[0];
        ...
        test('remove the handler', function() {
          ...
          assert.equal(removedObserver, handlerToBeRemoved);
        });
      });
      ...
      ['check-error-http-200', 'check-error-http-403', 'check-error-http-404',
       'check-error-http-500', 'check-error-2152398878']
        .forEach(function(error) {
          test(error, function() {
            MockNavigatorSettings.mTriggerObservers(geckoUpdateSetting, {
              settingValue: error
            });
            assert.equal(systemStatus.textContent, error);
          });
      });
    });
    ...
    suite('getting response for app update >', function() {
      suite('successful >', function() {
        var handlerToBeRemoved = null;
        setup(function() {
          handlerToBeRemoved = appsUpdateHandlers[0];
  ...
  suite('loadHardwareInfo >', function() {
    ...
    setup(function() {
      ...
      // Remove all default iccids
      MockNavigatorMozIccManager.iccIds.forEach(function(iccId) {
        MockNavigatorMozIccManager.removeIcc(iccId);
      });
