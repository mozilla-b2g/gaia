Evme.Utils = new function Evme_Utils() {
  var self = this,
      ...
      OSMessages = this.OSMessages = {
          ...
          "SWIPE_OUT_DISABLE": "swipe-disable",
          "SWIPE_OUT_ENABLE": "swipe-enable",
  ...
  this.sendToOS = function sendToOS(type, data) {
    switch (type) {
      case OSMessages.ON_INIT:
        return EvmeManager.onInit(Evme);
        break;
      ...
      default:
        self.log('Error sending message to OS: "' + type + '" (' + JSON.stringify(data) + ') doesn\'t exist!');
    }
  };
  ...
  this.switchAttrs = function switchAttributes(el, oldAttr, newAttr) {
...
Evme.$ = function Evme_$(sSelector, elScope, iterationFunction) {
  var isById = this.$regexById.test(sSelector),
      els = null;

  if (isById) {
    els = [document.getElementById(sSelector.replace('#', ''))];
  } else {
    els = (elScope || document).querySelectorAll(sSelector);
  }

  if (iterationFunction instanceof Function) {
