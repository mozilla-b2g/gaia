 ...
 * 2013-6-24
 *
 * Fred Lin remove the namespace for Firefox OS build
 *
 * remove _inlineImports since its not support in packaged app
 *
...
var CleanCSS = {
  ...
  process: function(data, options) {
    ...
    replace(/(animation|animation\-name|font|font\-family):([^;}]+)/g,
      function(match, propertyName, fontDef) {
      return propertyName + ':' + fontDef.replace(/['"]([\w\-]+)['"]/g, '$1');
    });

    // strip parentheses in @keyframes
    replace(/@(\-moz\-|\-o\-|\-webkit\-)?keyframes ([^{]+)/g,
      function(match, prefix, name) {
      prefix = prefix || '';
      return '@' + prefix + 'keyframes ' + (name.indexOf(' ') > -1 ?
        name : name.replace(/['"]/g, ''));
    });
    ...
    ['toHex', 'toName'].forEach(function(type) {
      ...
      var colorSwitcher = function(match, prefix, colorValue, suffix) {
        return prefix + CleanCSS.colors[type][colorValue.toLowerCase()] +
               suffix;
      };
    ...
    replace(/(font|font\-weight):(normal|bold)([ ;\}!])/g,
      function(match, property, weight, suffix) {
      if (weight == 'normal')
    ...
    // none to 0
    replace(/(border|border-top|border-right|border-bottom|border-left|outline):none/g,
      '$1:0');
    ...
    var shorthandRegex = function(repeats, hasSuffix) {
      var pattern = '(padding|margin|border\\-width|border\\' +
    ...
    replace(shorthandRegex(4),
      function(match, property, size1, size2, size3, size4) {
    ...
    replace(shorthandRegex(3, true),
      function(match, property, size1, size2, size3, suffix) {
    ...
    replace(shorthandRegex(2, true),
      function(match, property, size1, size2, suffix) {
      if (size1 === size2)
        return property + ':' + size1 + suffix;
      else
        return match;
    ...
    replace(new RegExp('__CSSCOMMENT__(' + lineBreak + '| )?', 'g'),
      function() {
    ...
    replace(function moveCharset() {
      ...
      data = firstCharset +
        ...
        data.replace(new RegExp('@charset [^;]+;(' + lineBreak +
          ')?', 'g'), '');
    });
  ...
  // Inlines all imports taking care of repetitions,
  // unknown files, and cilcular dependencies
  /*_inlineImports: function(data, options) {
    ...
    var inlinedFile = function() {
      ...
      if (existsSync(fullPath) && fs.statSync(fullPath).isFile() &&
        options.visited.indexOf(fullPath) == -1) {
        options.visited.push(fullPath);

        var importedData = fs.readFileSync(fullPath, 'utf8');
        var importRelativeTo = path.dirname(fullPath);
        importedData = CleanCSS._rebaseRelativeURLs(importedData,
          importRelativeTo, options._baseRelativeTo);
  ...
  _rebaseRelativeURLs: function(data, fromBase, toBase) {
    ...
    for (; nextEnd < data.length; ) {
      ...
      if (url[0] != '/' && url.indexOf('data:') !== 0 &&
        url.substring(url.length - 4) != '.css') {
        url = path.relative(toBase,
          path.join(fromBase, url)).replace(/\\/g, '/');
      }
  ...
  // Strip special comments (/*! ... */) by replacing them by
  // __CSSCOMMENT__ marker
  // for further restoring. Plain comments are removed.
  // It's done by scanning datq using
