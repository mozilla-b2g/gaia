...
suite('Specific apps should be present in given build configuration' +
  '[Network Require]', function() {
  ...
  var SPARK_APPS = [
    'Customizer', 'Hackerplace', 'Studio', 'Sharing', 'Webmaker',
    'Bugzilla Lite', 'Facebook', 'Twitter', 'RunWhatsApp (Preview)', 'BuddyUp',
    'Notes', 'Calculator', 'SWOOOP'
  ];
  ...
  var SCENARIOS = [
    // Engineerings apps are not described in any file of the homescreen.
    // These scenarios don't work currently. See bug 1187330 for details
  {
    command: PRODUCTION_CONFIG + MAKE_COMMAND,
    expectedApps: _.union(REGULAR_APPS)
  }, {
    command: PRODUCTION_CONFIG + SPARK_CONFIG + MAKE_COMMAND,
    expectedApps: _.union(REGULAR_APPS, SPARK_APPS)
  }];
