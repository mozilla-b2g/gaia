...
suite('settings.js', function() {
  ...
  setup(function() {
    ...
    mockUtils.getUrlObj = function(origin) {
      return {
        host: origin + '_host',
  ...
  suite('setConfig, genStageWebappJSON, fillExternalAppManifest', function() {
    ...
    setup(function() {
      ...
      mockUtils.getFile = function(mainPath, subPath) {
    ...
    test('setConfig', function() {
      ...
      assert.equal(webappManifest.stageDir, mockConfig.GAIA_DIR +
        'build_stage');
    });
    ...
    test('manifestInterAppHostnames', function() {
      var mockManifest = {
        connections: [{
          rules: {
            manifestURLs: ['http://testManifestURL.gaiamobile.org']}
        }]
      };
      webappManifest.setConfig(mockConfig);
      var newmanifest = webappManifest.manifestInterAppHostnames(mockManifest);
      assert.deepEqual(newmanifest, {
        connections: [{
          rules: {
            manifestURLs: [mockConfig.GAIA_SCHEME + 'testManifestURL.' +
                           mockConfig.GAIA_DOMAIN + mockConfig.GAIA_PORT]
          }
        }]
      });
    });

    test('addToWebapps', function() {
      var expectedManifestContent = 'testmanifestContent';
      webappManifest.setConfig(mockConfig);
      webappManifest.manifests[mockWebapp.domain] = expectedManifestContent;
      webappManifest.addToWebapps(mockWebapp);
      assert.deepEqual(webappManifest.webapps, {
        'sourceDirectoryName': mockWebapp
      });
    });
  });
});
