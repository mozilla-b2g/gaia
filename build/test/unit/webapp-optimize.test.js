...
suite('webapp-optimize.js', function() {
  ...
  suite('WebappOptimize', function() {
    ...
    test('optimzeFiles, optimize all other files except with aggregatePrefix' +
         ' when all files are proceeded', function() {
      var result;
      webappOptimize.config = mockConfig;
      webappOptimize._optimizeFile = function(files) {
        result = files;
      };
      fileChildren.push(
        mockUtils.getFile('gaia-test-prefix-test.js'));
      fileChildren.push(
        mockUtils.getFile('test.js'));
      webappOptimize.webapp = {
        buildDirectoryFile: mockUtils.getFile('build.js')
      };
      webappOptimize.optimizeFiles();
      assert.equal(result.leafName, 'test.js', 'should left all .js files' +
        ' exclude those with gaia-test-prefix prefix');
    });

    test('_optimizeFile, optimize file by its type', function() {
      var file = {
        leafName: 'test.js',
        content: 'test-content'
      };
      webappOptimize._optimizeFile(file);
      assert.deepEqual(writeFile, file, 'should write new js file');
      assert.equal(writeFileContent, 'minified-' + file.content,
        'should write new js minified content');
    });

    test('writeDictionaries', function() {
      isFileExist = true;
      webappOptimize.config = mockConfig;
      fileChildren = [mockUtils.getFile(mockDictionary[0] + '.json')];
      webappOptimize.webapp = {
      ...
      assert.equal(writeFile.leafName, 'build_stage/locales-obj',
    ...
    test('execute, main function of webappOptimize', function() {
      ...
      assert.equal(webappOptimize.numOfFiles, 1,
        'should only have one file left');
  ...
  suite('HTMLOptimizer', function() {
    ...
    test('inlineCSSResources', function() {
      ...
      assert.equal(modifiedDOMs[0].innerHTML,
        'minified-content-document/link[rel="stylesheet"]-href');
    });
