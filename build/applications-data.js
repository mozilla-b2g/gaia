...
function execute(options) {
  ...
  (function customizeMaximumImageSize() {
    let defaultValue = {
      ...
      maxGifImageFileSize: 2 * 1024 * 1024
    };

    // On low-memory devices like Tarako, we need different default values
    if (config.GAIA_MEMORY_PROFILE === 'low') {
      defaultValue.maxImagePixelSize = 2 * 1024 * 1024;
      defaultValue.maxSnapshotPixelSize = 1600 * 1200; // Tarako camera size
      defaultValue.maxGifImageFileSize = 1024 * 1024; // 1 mb
    ...
    let content =
      ...
      '//\n'+
      '// To customize these values, create a camera.json file in the\n' +
      ...
      '// unknown.\n' +
      '//\n' +
      '// Set maxPickPixelSize to specify a maximum size for the images\n' +
      '// returned by Camera and Gallery pick activities. This can\n' +
      '// be useful for low-memory devices like Tarako\n' +
      '//\n' +
