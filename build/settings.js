...
if (GAIA_DEV_PIXELS_PER_PX != '1') {
  wallpaper_filename = 'build/wallpaper@' + GAIA_DEV_PIXELS_PER_PX + 'x.jpg';
} else {
...
settings['homescreen.manifestURL'] = gaiaManifestURL('homescreen');

// Set the ftu manifest URL
if (!NOFTU)
  settings['ftu.manifestURL'] = gaiaManifestURL('communications');

settings['language.current'] = GAIA_DEFAULT_LOCALE;
let file = Gaia.sharedFolder.clone();
file.append('resources');
file.append('keyboard_layouts.json');
keyboard_layouts_res = getJSON(file);
keyboard_layouts = keyboard_layouts_res['layout'];
keyboard_nonLatins = keyboard_layouts_res['nonLatin'];
if (GAIA_DEFAULT_LOCALE in keyboard_layouts) {
  default_layout = keyboard_layouts[GAIA_DEFAULT_LOCALE]
  if (!(GAIA_DEFAULT_LOCALE in keyboard_nonLatins)) {
    settings['keyboard.layouts.english'] = false
  }
  settings['keyboard.layouts.' + default_layout] = true;
}
settings['devtools.debugger.remote-enabled'] = REMOTE_DEBUGGER == true;
...
function setWallpaper(callback) {
  ...
  let wallpaper = getAbsoluteOrRelativePath(wallpaper_filename);
...
function setRingtone(callback) {
  ...
  let ringtone_name = 'shared/resources/media/ringtones/ringer_classic_courier.opus';
  let ringtone = getAbsoluteOrRelativePath(ringtone_name);
...
function setNotification(callback) {
  ...
  let notification_name = 'shared/resources/media/notifications/notifier_bell.opus';
  let notification = getAbsoluteOrRelativePath(notification_name);
...
function overrideSettings() {
  ...
  let override = getAbsoluteOrRelativePath(SETTINGS_PATH);
  if (override.exists()) {
    let content = getJSON(override);
...
function writeSettings() {
  ...
  let settingsFile = getFile(PROFILE_DIR, 'settings.json');
  let content = JSON.stringify(settings);
  writeContent(settingsFile, content + '\n');
}
